...
```{r evolucao_particulado2.5,message=FALSE,warning=FALSE}
# Opção pelo ggplot convencional.
dados_poluicao %>% filter(year <= 2016) %>% mutate(year = as.factor(year)) %>% group_by(station,hour,year) %>% summarise(mediaPM25 = mean(PM2.5,na.rm = T)) %>% ggplot(aes(hour,mediaPM25,color=year)) + geom_line() + scale_y_log10() + facet_wrap(station~.) + scale_color_brewer(palette="Dark2")
```
